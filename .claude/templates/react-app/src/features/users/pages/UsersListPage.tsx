...
export default function UsersListPage() {
    // Replace with: const { data, isLoading } = useUserList()
            ...
            <Card flush>
                ...
                <div className="flex items-center gap-3 flex-wrap px-(--pad) pb-4">
                    <div className="flex items-center gap-2 border border-(--field-border) bg-(--field) rounded-(--r-sm) px-3 h-9 flex-1 min-w-[200px] max-w-[320px]">
                        ...
                        <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search users…" className="bg-transparent border-none outline-none text-[13.5px] text-(--ink) placeholder:text-(--faint) w-full" />
                    </div>
                    <div className="flex flex-wrap gap-2">
                        {ROLE_FILTERS.map((f) => (
                            <button key={f} onClick={() => setRoleFilter(f)} className={["px-3 h-8 rounded-(--r-sm) text-[13px] border transition-colors capitalize", roleFilter === f ? "bg-(--accent) text-(--accent-ink) border-(--accent) font-semibold" : "bg-transparent text-(--muted) border-(--border) hover:bg-(--surface-2) hover:text-(--ink)"].join(" ")}>
                                {f === "all" ? `All ${users.length}` : ROLE_LABEL[f as UserRole] + "s"}
                            </button>
                        ))}
                    </div>
                </div>
                            ...
                            <TableRow key={user.id}>
                                ...
                                <TableCell>
                                    <div className="flex items-center justify-end gap-1">
                                        <Link to={`/admin/users/${user.id}/edit`} className="w-[34px] h-[34px] flex items-center justify-center rounded-(--r-sm) border border-transparent text-(--muted) hover:bg-(--surface-2) hover:text-(--ink) hover:border-(--border) transition-colors" title="Edit">

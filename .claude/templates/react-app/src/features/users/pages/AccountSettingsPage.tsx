            ...
            <Card>
                ...
                <FormRow label="Avatar" noBorderTop>
                    <div className="flex items-center gap-4">
                        <div className="shrink-0 rounded-full border border-(--border) bg-(--surface-2) text-(--ink) font-semibold grid place-items-center overflow-hidden" style={{ width: 80, height: 80, fontSize: 24, letterSpacing: "-0.01em" }}>
                            {avatarSrc ? <img src={avatarSrc} alt="Avatar preview" className="w-full h-full object-cover block" /> : initials}
                        </div>
                        <div className="min-w-0">
                            <input ref={fileRef} type="file" accept={AVATAR_ACCEPT.join(",")} onChange={onAvatarPick} className="hidden" />
                            <div className="flex gap-2 mb-2">
                                <Button size="sm" leftIcon={<Upload01Icon size={14} strokeWidth={1.8} />} isLoading={isUploadingAvatar} onClick={() => fileRef.current?.click()}>
                                    Upload new
                                </Button>
                                <Button size="sm" variant="ghost" disabled={!avatarSrc} onClick={onAvatarRemove}>
                                    Remove
                                </Button>
                            </div>
            ...
            <Card>
                ...
                <form onSubmit={profileForm.handleSubmit(onProfileSubmit)} noValidate>
                    <FormRow label="Full name" help="Shown across the workspace and on your posts." noBorderTop>
                        <Input placeholder="Your name" error={profileForm.formState.errors.name?.message} {...profileForm.register("name")} />
                    </FormRow>
            ...
            <Card>
                ...
                <form onSubmit={passwordForm.handleSubmit(onPasswordSubmit)} noValidate>
                    <FormRow label="Current password" noBorderTop>
                        <PasswordInput placeholder="Enter current password" showToggle={false} error={passwordForm.formState.errors.currentPassword?.message} {...passwordForm.register("currentPassword")} />
                    </FormRow>

                    <FormRow label="New password">
                        <PasswordInput placeholder="Enter new password" showToggle={false} error={passwordForm.formState.errors.newPassword?.message} {...passwordForm.register("newPassword")} />
                        {/* Strength meter */}
                        <div className="flex gap-[5px] mt-[9px]">
                            {[0, 1, 2, 3].map((i) => (
                                <span key={i} className="h-1 flex-1 rounded-[3px] transition-colors duration-150" style={{ background: i < score ? "var(--accent)" : "var(--surface-2)" }} />
                            ))}
                        </div>
                        <p className="text-[12.5px] text-(--faint) mt-[7px]">{newPasswordValue ? `${STRENGTH_LABEL[score]} — at least 8 characters with a number and a symbol.` : "At least 8 characters with a number and a symbol."}</p>
                    </FormRow>

                    <FormRow label="Confirm password">
                        <PasswordInput placeholder="Re-enter new password" showToggle={false} error={passwordForm.formState.errors.confirmPassword?.message} {...passwordForm.register("confirmPassword")} />
                    </FormRow>

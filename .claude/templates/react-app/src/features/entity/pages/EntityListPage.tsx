...
export default function EntityListPage() {
    // Replace with: const { data, isLoading } = useEntityList()
            ...
            <Card flush>
                ...
                <div className="flex items-center gap-3 flex-wrap px-(--pad) pb-4">
                    <div className="flex items-center gap-2 border border-(--field-border) bg-(--field) rounded-(--r-sm) px-3 h-9 flex-1 min-w-[200px] max-w-[320px]">
                        ...
                        <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search entities…" className="bg-transparent border-none outline-none text-[13.5px] text-(--ink) placeholder:text-(--faint) w-full" />
                    </div>
                    <div className="flex gap-2">
                        {(["all", "published", "draft"] as const).map((f) => (
                            <button key={f} onClick={() => setStatusFilter(f)} className={["px-3 h-8 rounded-(--r-sm) text-[13px] border transition-colors capitalize", statusFilter === f ? "bg-(--accent) text-(--accent-ink) border-(--accent) font-semibold" : "bg-transparent text-(--muted) border-(--border) hover:bg-(--surface-2) hover:text-(--ink)"].join(" ")}>
                                {f === "all" ? `All ${rows.length}` : f.charAt(0).toUpperCase() + f.slice(1)}
                            </button>
                        ))}
                    </div>
                </div>
                    ...
                    <TableBody>
                        {filtered.map((row) => (
                            <TableRow key={row.id}>
                                ...
                                <TableCell>
                                    <Badge variant={row.status === "published" ? "success" : "neutral"}>{row.status === "published" ? "Published" : "Draft"}</Badge>
                                </TableCell>
                                <TableCell className="text-[13px] text-(--muted) tabular-nums">{row.createdAt}</TableCell>
                                <TableCell>
                                    <div className="flex items-center justify-end gap-1">
                                        <Link to={`/admin/entities/${row.id}/edit`} className="w-[34px] h-[34px] flex items-center justify-center rounded-(--r-sm) border border-transparent text-(--muted) hover:bg-(--surface-2) hover:text-(--ink) hover:border-(--border) transition-colors" title="Edit">
                                            <PencilEdit01Icon size={16} strokeWidth={1.8} />
                                        </Link>
                                        <button className="w-[34px] h-[34px] flex items-center justify-center rounded-(--r-sm) border border-transparent text-(--muted) hover:bg-red-500/10 hover:text-red-400 hover:border-red-500/20 transition-colors" title="Delete">
                ...
                <CardFooter>
                    <span className="text-[13px] text-(--faint)">
                        Showing 1–{filtered.length} of {rows.length} entries
                    </span>

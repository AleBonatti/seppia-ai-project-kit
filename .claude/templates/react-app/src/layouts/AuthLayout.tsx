...
export function AuthLayout({ children, title = "Project Name" }: AuthLayoutProps) {
    ...
    return (
        <div className="min-h-screen grid place-items-center" style={{ background: "var(--bg)", color: "var(--ink)", padding: 32 }}>
            {/* Theme toggle */}
            <button type="button" onClick={() => setTheme(theme === "dark" ? "light" : "dark")} title="Toggle theme" aria-label="Toggle theme" className="fixed top-[18px] right-[18px] w-10 h-10 grid place-items-center rounded-[10px] border text-(--muted) cursor-pointer hover:text-(--ink) transition-colors" style={{ background: "var(--box)", borderColor: "var(--border)" }}>
                {theme === "dark" ? <Sun03Icon size={19} strokeWidth={1.8} /> : <Moon02Icon size={19} strokeWidth={1.8} />}
            </button>
            ...
            <div
                ...
                <div className="flex items-center justify-center gap-2 mb-[30px]">
                    <span className="w-[46px] h-[46px] grid place-items-center" style={{ color: "var(--accent)" }}>

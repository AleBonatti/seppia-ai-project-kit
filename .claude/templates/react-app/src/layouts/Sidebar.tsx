...
import { DashboardSquare01Icon, InsertColumnLeftIcon, Moon02Icon, Sun03Icon, Logout01Icon, ArrowUp01Icon, Settings01Icon, UserAccountIcon } from "@/lib/icons";
...
export function Sidebar() {
    ...
    const [collapsed, setCollapsed] = useState(() => localStorage.getItem("sidebar") === "icononly");
    ...
    return (
        <aside className="flex flex-col overflow-visible relative" style={{ background: "var(--sidebar)", padding: "18px 11px", gap: 4 }}>
            {/* Brand */}
            <div className={cn("flex items-center gap-[5px] px-2 pb-[18px]", collapsed && "flex-col gap-3.5 px-0")}>
                {/* Logo mark — replace SVG with project logo if needed */}
                {!collapsed && (
                    <span className="flex-none grid place-items-center" style={{ width: 34, height: 34, color: "var(--accent)" }}>
                ...
                {!collapsed && <span className="flex-1 font-semibold text-2xl tracking-[-0.03em] text-(--ink) whitespace-nowrap truncate">Project Name</span>}
                <button type="button" onClick={toggleCollapse} className={cn("flex-none w-[30px] h-[30px] grid place-items-center rounded-lg bg-transparent text-(--muted) border-none cursor-pointer hover:bg-(--surface-2) hover:text-(--ink) transition-colors", collapsed && "mx-auto")} aria-label={collapsed ? "Expand menu" : "Collapse menu"}>
                    <InsertColumnLeftIcon size={19} strokeWidth={1.8} />
                </button>
            </div>
            ...
            {NAV.filter((g) => g.items.length > 0).map(({ group, items }) => (
                <div key={group}>
                    ...
                    {collapsed ? <div className="mx-3.5 my-2 mt-[15px] h-px" style={{ background: "color-mix(in srgb, var(--faint) 42%, transparent)" }} /> : <p className="text-[10.5px] font-semibold uppercase tracking-[.09em] text-(--faint) px-3 pt-4 pb-[7px] whitespace-nowrap overflow-hidden">{group}</p>}
                    <div>
                        {items.map((item) => (
                            <NavLink key={item.href} to={item.href} title={item.label} className={({ isActive }) => cn("flex items-center gap-3 rounded-(--r-sm) px-3 py-[9px] text-[14.5px] font-medium transition-colors relative border-none w-full text-left cursor-pointer no-underline", collapsed && "justify-center px-0 py-[11px]", isActive ? "text-(--selected) bg-(--surface-2) font-semibold" : "text-(--muted) hover:bg-(--surface-2) hover:text-(--ink)")}>
                                {({ isActive }) => (
                                    <>
                                        {isActive && <span className="absolute left-[-14px] top-[9px] bottom-[9px] w-[3px] bg-(--accent) rounded-r-[3px]" />}
                                        ...
                                        {!collapsed && (
                                            <>
                                                ...
                                                {item.count !== undefined && <span className={cn("text-[11px] font-semibold tabular-nums px-2 py-px rounded-full min-w-6 text-center", isActive ? "bg-(--accent) text-(--accent-ink)" : "bg-(--surface-2) text-(--muted)")}>{item.count}</span>}
                                            </>
                                        )}
                                    </>
                                )}
            ...
            <div className="mt-auto pt-3">
                ...
                {menuOpen && <div className="fixed inset-0 z-55" onClick={() => setMenuOpen(false)} />}

                {/*
          Wrapper clips the panel while it's translated below.
          Height accounts for the usercard (66px) + the menu body.
        */}
                <div className="relative overflow-hidden" style={{ height: menuBodyH + 66 }}>
                    {/*
          ...
          */}
                    <div
                        className={cn("absolute left-0 right-0 bottom-0 z-60 flex flex-col rounded-(--r) border", "transition-[transform,background,border-color,box-shadow] duration-340", menuOpen ? "bg-(--box) border-(--border) shadow-(--shadow) translate-y-0" : "bg-transparent border-transparent shadow-none")}
                        style={{
                        ...
                        <button type="button" onClick={() => setMenuOpen((o) => !o)} className={cn("flex w-full items-center gap-2.5 p-2 cursor-pointer bg-(--box) border border-(--border) rounded-(--r) text-left text-(--ink) transition-colors", menuOpen ? "rounded-b-none border-transparent hover:bg-(--box)" : "hover:bg-(--surface-2)", collapsed && "justify-center p-[7px] w-10 h-10 mx-auto box-content")}>
                            ...
                            {!collapsed && (
                                <>
                                    ...
                                    <span className="text-(--muted) flex-none transition-transform duration-180" style={{ transform: menuOpen ? "rotate(0deg)" : "rotate(180deg)" }}>
